    ...
    {	
        ...
        var Cascade=function(cascade,page,options,requestCurrent)
        {
			...
			{
				if($this.options.twitter.name.length==0)
				{
					$('.latest-tweets').css('display','none');
					return;
				}
				
				$.getJSON('http://api.twitter.com/1/statuses/user_timeline/'+$this.options.twitter.name+'.json?count='+$this.options.twitter.count+'&callback=?',function(data) 
				{
					if(data.length)
					{
						var list=$('<ul>');
